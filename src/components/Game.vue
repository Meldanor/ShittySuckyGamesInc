<template>
  <div class="mb-10 m-2 shadow-lg border-gray-800 bg-gray-100 rounded-md">
    <div class="h-14 grid grid-cols-8 gap-1">
      <div class="text-lg font-bold text-center col-span-6">{{game.name}}</div>
      <div class="text-lg col-span-2 flex flex-col pt-1">
        <div class="self-center flex justify-center items-center h-12 w-12 ring-4 rounded-full ring-green-300 bg-green-500">{{game.score}}</div>
      </div>
    </div>
    ...
    <p class="px-1 pt-1 cursor-pointer text-sm" :class="{'line-clamp-none': isClicked, 'line-clamp-4': !isClicked}" @click="onClick">
      {{game.text}}
    </p>
  </div>
</template>
...
export default {
  ...
  computed: {
    releaseDate() {
      return `W${this.game.releaseDate.week} M${this.game.releaseDate.month} ${this.game.releaseDate.year}`; 
    }
  }
}
</script>
